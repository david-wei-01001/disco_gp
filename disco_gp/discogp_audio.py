...
from .data import setup_audio_task
...
def load_head(head_name: str = None):
    pass


class DiscoGPAudioEncoder(nn.Module):
    ...
    def forward(self, tokens, return_states: bool = False):
        ...
        residual = einops.rearrange(tokens, "batch position d_model -> batch position 1 d_model")
    ...
    @classmethod
    def from_pretrained(cls, cfg, head_name=None):
        ...
        wrapped_model = HookedAudioEncoder.from_pretrained(cfg.full_model_name)
        ...
        if head_name:
            model.head = load_head(head_name)
    ...
    def compute_faith_loss(self, batch_logits_masked, batch_inputs, original_logits=None):
        ...
        """
        if self.cfg.task_type in ['spoof']:
            return self.compute_faith_loss_binary_label(batch_logits_masked, batch_inputs, original_logits)
    ...
    def compute_complete_loss(self, batch_logits_masked, batch_inputs):
        ...
        if self.cfg.task_type in ['spoof']:
            return self.compute_complete_loss_binary_label(batch_logits_masked, batch_inputs)
            # elif self.cfg.task_type in ['pararel']:
            #     batch_logits_masked = batch_logits_masked[:, :, self.answer_idx_vocab]
    ...
    def compute_faith_loss_binary_label(self, batch_logits_masked, batch_inputs, original_logits=None):
        """
        Implements the pseudocode: produce log-sigmoid from self.head, compute prediction,
        compute a faithful loss (BCEWithLogits), optional KL between original_logits and new logits,
        and count number correct.

        Args:
            batch_logits_masked: Tensor (B, ...)  -- input features to the head (or pre-head if you change)
            batch_inputs: dict with keys:
                - 'labels': (B,) tensor with 0/1 labels
                - 'seq_lens': (B,) sequence lengths (kept for compatibility)
            original_logits: optional baseline logits (either shape (B,) single-logit or (B,2) class logits)
        Returns:
            dict with keys: 'faith' (loss tensor), 'kl_div' (CPU scalar tensor or None), 'n_correct' (int)
        """

        # read seq_lens and batch size (kept from your proto)
        batch_seq_lens = batch_inputs.get('seq_lens', None)
        batch_size = batch_logits_masked.shape[0]

        # produce single-logit outputs via the head
        logits = self.head(batch_logits_masked).squeeze(-1)  # expect (B,1) or (B,)

        # log-probability of positive class (log-sigmoid) and probability
        log_probs_target_masked = F.logsigmoid(logits)  # (B,)
        probs_target_masked = torch.sigmoid(logits)     # (B,)

        # predictions: 1 if prob>0.5 else 0
        batch_pred = (probs_target_masked > 0.5).long()  # (B,)

        # ground-truth labels (ensure on same device)
        labels = batch_inputs['labels'].float().to(logits.device)

        # faithful loss: for single-logit binary output use BCEWithLogits
        batch_faith_loss = F.binary_cross_entropy_with_logits(logits, labels, reduction='mean')

        # optional KL: compute D_KL(p_orig || p_new) (averaged over batch)
        batch_kl = None
        if original_logits is not None:
            batch_kl = F.kl_div(
                original_logits.to(batch_logits_masked.device).log_softmax(dim=-1),
                batch_logits_masked.log_softmax(dim=-1),
                log_target=True
            ).cpu()

        # number correct
        batch_n_correct = int((batch_pred == labels).sum().item())
    ...
    def compute_complete_loss_binary_label(self, batch_logits, batch_inputs):
        """
        batch_logits: (B, seq_len, ...)  -- features you pass into self.head
        batch_inputs: dict with 'seq_lens' (unused) but kept for signature
        Returns: (loss, logits_gb) where logits_gb is detached logits on CPU
        """
        # produce single-logit outputs via the head (expect shape (B,1) or (B,))
        logits = self.head(batch_logits).squeeze(-1)   # (B,)
        device = logits.device

        # uninformative target: probability 0.5 for every example
        target_uniform = torch.full_like(logits, 0.5, device=device)  # floats in [0,1]

        # Use BCEWithLogits (logits in, probs target) — do NOT sigmoid first
        batch_complete_loss = F.binary_cross_entropy_with_logits(logits, target_uniform, reduction='mean')

        # a gradient-blocked copy for logging / return
        logits_gb = logits.detach().cpu()
